      ...
      try {
        ...
        form.parse(req, (err, fields, files) => {
          ...
          if (files.file) {
            ...
            let filename = dayjs().format('YYYY/MM/DD/') + uuid.v1().replace(/-/g, '') + path.extname(files.file[0].path)
...
const fileMoveAli = async (filename, path) => {
  let result = await aliclient.put(filename, path)
  return result
}
